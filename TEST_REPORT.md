...
| Vitest 全量测试 | 23 个文件，121 项通过，0 项失败 |
| V8 覆盖率门槛 | 通过 |
| Vite 生产构建 | 通过 |
| PWA Service Worker | 生成成功，预缓存 18 项（1621.12 KiB） |
| npm 高危依赖审计 | 0 个漏洞 |
| 本地真实 Chrome 验证 | 正式首页、词语花园桌面与 390px 窄屏通过人工视觉审阅；浏览器错误为空 |
...
| 范围 | 语句 | 分支 | 函数 | 行 |
|---|---:|---:|---:|---:|
| 全部生产代码 | 89.20% | 82.15% | 86.93% | 92.43% |
| `src/core` | 88.27% | 78.35% | 92.04% | 91.25% |
| `src/pages` | 89.19% | 83.81% | 82.25% | 92.70% |
...
| `src/test/architecture-contract.test.ts` | 14 | Typing/Match 输入边界、随机选词、错题本、奖励、存储、音频和规则版本 |
| `src/test/audio-service.test.ts` | 6 | 设置持久化、静音同步、输出注销、页面隐藏和语音 ducking |
| `src/test/challenge-progression.test.ts` | 6 | 青蛙阶段、追踪参数及连连看 4/6/8 对规则与计时 |
| `src/test/game-pages-routing.test.tsx` | 1 | training、frog、chase、match 页面分派 |
| `src/test/game-pages.test.tsx` | 13 | 键盘游戏计时、公平暂停、阶段升级、检查点、街区、保存和降级 |
...
| `src/test/match-game-page.test.tsx` | 7 | 错配不回退、三关通关、分级词包、计时耗尽、暂停、重开和退出保存 |
...
- 三关依次为 4 对不限时、6 对 120 秒、8 对 90 秒，词汇难度逐步覆盖 1～2、1～3、1～5 级；
...
- Frog 每 8 词进入下一阶段但不自动结算，无尽局内难度只升不降；
...
- Vite 8.2.2 构建成功，43 个模块完成转换；
- PWA `generateSW` 成功，预缓存 18 项（1621.12 KiB）；
